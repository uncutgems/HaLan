   ...
   if(event is DriverLocationEventGetLocation){
     ...
     final String token = await repository.getTokenFirebase();
      timer = Timer.periodic(const Duration(seconds: 10), (Timer timer) async {
        final Response response = await get('https://dobody-anvui.firebaseio.com/trips/$tripId.json?auth=$token');
//        if (response != null) print('response: ' + response.body);
        ...
        if (data != null) {
          ...
          if (data[Constant.tripInfo] != null &&
            ...
            final Map<String,dynamic> tripJson = data[Constant.tripInfo] as Map<String,dynamic>;

            final List<dynamic> drivers = tripJson[Constant.drivers] as List<dynamic>;
            driver = User.fromMap(drivers.first as Map<String,dynamic>);
          }
          if (data[Constant.tripInfo] != null && data[Constant.tripInfo][Constant.vehicle] != null) {
            vehicle = Vehicle.fromMap(data[Constant.tripInfo][Constant.vehicle] as Map<String,dynamic>);
          }
